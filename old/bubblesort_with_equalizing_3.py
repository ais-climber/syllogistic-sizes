...
def SiConstructionEQUALITY(indices, listOfLists):
	...
	for m in range(len(flattened) - 1):
		for equalGroup in listOfLists[::-1]:
			# Equalize the current group.
			# TODO

			# After equalizing, ensure that the adjacent pairs at this group's
			# leftmost border have the appropriate size difference.

			# (i, j) is the last element of the preceding group, and (k, l)
			# is the first element of this one.  We want Si U Sj < Sk U Sl.
			(i, j) = listOfLists[(listOfLists.index(equalGroup) - 1)][-1]
			(k, l) = equalGroup[0]

			# We pump elements in order to separate Si U Sj < Sk U Sl.
